...
from .filters import ProductCategoryFilter, ProductCollectionFilter, ProductListFilter
...
from .utils import (
    ...
    products_with_details, products_for_products_list_order_by_publication)
...
def category_index(request, slug, category_id):
    ...
    return TemplateResponse(request, 'catecatecategorygorycategorygory/index.html', ctx)
...
def product_list(request):
    ...
    sorted_products = products_for_products_list_order_by_publication(user=request.user)
    product_filter = ProductListFilter(
        request.GET, queryset=products)
    ctx = get_product_list_context(request, product_filter)
    ctx.update({'new_products': sorted_products})
    return TemplateResponse(request, 'all-products/index.html', ctx)
